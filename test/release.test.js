...
test("bumps stable semantic versions", async () => {
  ...
  assert.equal(bumpVersion("0.0.5", "patch"), "0.0.6");
  assert.equal(bumpVersion("0.0.5", "minor"), "0.1.0");
  assert.equal(bumpVersion("0.0.5", "major"), "1.0.0");
  assert.throws(() => bumpVersion("0.0.5-beta.1", "patch"), /stable semantic version/);
});

test("updates package and lockfile versions together", async () => {
  const { writeReleaseVersion } = await loadReleaseTools();
  const directory = await mkdtemp(join(tmpdir(), "unity-semver-release-"));
  const packagePath = join(directory, "package.json");
  const lockPath = join(directory, "package-lock.json");
  await writeFile(packagePath, '{"name":"example","version":"0.0.5"}\n');
  await writeFile(lockPath, '{"name":"example","version":"0.0.5","packages":{"":{"name":"example","version":"0.0.5"}}}\n');
